...
Short description of (1) the features you compute for each rps-string and 
      ...
      based on multiple runs of the machine generated 
      I used the longest non-repeating substring algorithm and then
      from each longest non-repeating 
"""
...
mygenerated = ['rpsprpsprppsppspsssspprpsppsprsppsprpspprsprpspsprpspspsrpssrpspsprpspsrpsprpspsrpspppppspspspssspprsppsrpsprpsrsprprprprpsrrrrrpssssssrrssppsssspsssssppprrsssrrppsrrpsrprpsprpsrp', 'rpsprpsrpsrppprprprprppppppssssssrpspprpsprpsrrpprprpsprprpppsssrrrpsprppsrspprpsprpsprpsprpsprpsprpsppprpsprpsprpsprpspsppsppsspprpspspprsr', 'srpspppsprpsprpsprpsprppppppspppppspppspssssspssppspsppppppspppssprrpsprrrrrpspsrpspprrpsprpsprprpsprpsprpsprpsprpsprpsprrsrsprprprspsprrprps', 'srpspspssppspsspspspssprrpprrprrpsprppsrprsprpsrpsrpsprpsrpsrpsprpsprpsprprprsppsssssrpsprprpsprpspsprpsprpsrpsrpsprpprprprprprprpsrspsrprpsrprsp', 'sprpsprprprrprpsprprrpsprrrrpsprrppsrprrrsprrppssssssrpspppprpsppprrrrpsrpppprsppprssssrpprsprprprpsrpsprprsrssrpprrrrpsprprprprprprprprrpspsprppsrsprpsprpsprpsppsprps','rprsprpprprprprprprprppppssssssssrppppppprrrrpsprprprrrrrrppssssprrrrppppprrpsrrrrsprprprpprppsrprprrrrrrppppppsrsrrppssssrrrsppppprssssrppppsrppppsrppssrpppprspppsssssrpprprprpssrsssrsrsrsrssrprpprrp', '']
...
def mRSinLRS(string, lLim=4, cLim=2):
    ...
    cString = string
    currScore = 1000 
    while currScore > lLim: 
        cString = substring
        substring = longestRepeatedSubstring(cString)
        if len(substring) > 1:
            #currScore = round(math.sqrt(len(substring) * (cString.count(substring))**2)) 
            currScore = int(math.sqrt(len(substring)**3 + cString.count(substring)**2))
        else:
            currScore = 0
        # print("curr substring: ", substring)
    if(string.count(cString) > cLim):
        return (cString, string.count(cString))
    return False
"""
def longestConsecutive(s, alpha='rps'):
    maxL = [0] * len(alpha)
    for i in range(1, len(s)):
        ...
        if s[i] == s[i-1]:
            ...
            if cMaxL > maxL[s[i-1]]: #and s.count(s[i-1]*cMaxL) > 2:
                maxL[s[i-1]] = cMaxL
            cMaxL = 1
    maxLL = 1
    for i in maxL:
    ...
    for l in range(len(alpha)):
        ...
        for i in range(1, len(s)):
            if s[i] == alpha[l]:
                ...
                test = round((c**3 + currMaxL**4)**(1/3)) 
                # test = c*2
                # test += currMaxL*2
                # test = round(math.sqrt(test))
                if currMaxL > cLim and currMaxL > maxL[l] and test > mLim:
                #if currMaxL > mLim and currMaxL > maxL[l] and (currMaxL > 5 or s.count(alpha[l]*currMaxL) > cLim):
                    maxL[l] = c*currMaxL
                currMaxL = 0
...
def score_find(data, times=1):
    ...
    for i in range(len(data)):
        ...
        for j in range(times):
            try:
                t = mRSinLRS(cString, 10, 2)
                if j == 0:
                    subStrings[i] = [t]
                else:
                    subStrings[i].append(t)
                if subStrings[i][j]:
                    scores[i] += math.sqrt(len(subStrings[i][j][0]) * (subStrings[i][j][1]-1)**2)
                    cString = re.sub(subStrings[i][j][0], 'y', cString)
            except TypeError:
                print("type error:", subStrings[i], t, scores[i], cString)
        # print(scores[i]*50)
        tmp = scores[i]*550 // len(data[i])
        scores[i] = (int(scores[i]*550) + (longestConsecutive(data[i])*10))//len(data[i])
...
breakPt = round(scores2[(len(scores2))//2])
...
write_to_csv("result3.csv", result)
